...
/**
 * Ctrl without db check
 */
abstract class MiniCtrl
{
	...
	public static function beforeRoute ()
	{
		...
		if (empty ($database ["name"]) || empty ($database ["user"]) || empty ($database ["password"])) {
			die("'database' conf in 'conf/user.ini' is missing : please fill-in values");
		}
		...
		if (!empty ($database ["user"])) {
			$db = new SQL (
				$database ["type"] . ":host=" . $database ["host"] . ";port=" . $database ["port"] . ";dbname=" . $database ["name"],
